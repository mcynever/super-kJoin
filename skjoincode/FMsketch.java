...
public class FMsketch extends GeneralDataStructure {
	...
	public void encodevCMSegment(long flowID, long elementID, int[] s, int ss, int w, int segmentLength) {
		...
		int hash_val = GeneralUtil.FNVHash1(elementID^66666);
		int leadingZeros = Math.min(Integer.numberOfLeadingZeros(hash_val), FMsketchSize - 1);
		FMsketchMatrix[i*segmentLength+j%segmentLength].set(leadingZeros);
	}
	...
	public GeneralDataStructure getspreadnoise(GeneralDataStructure[] gds) {
		...
		int w = this.FMsketchMatrix.length;
	...
	public GeneralDataStructure getvCMminValueSegment(GeneralDataStructure[] gds, long flowid, int[] S, int[] SS, int w, int segmentLength) {
		FMsketch[] fm=(FMsketch[]) gds;
		w /= segmentLength;
		for(int i=0; i<FMsketchMatrix.length/segmentLength;i++) {
			int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowid) ^ S[i] ^ SS[0]) % w + w) % w;
			for (int pp = 0; pp<segmentLength; pp++) {
				this.FMsketchMatrix[i*segmentLength+pp]=fm[0].FMsketchMatrix[j*segmentLength+pp];

			}
				
		}
		for (int i = 1; i < gds.length; i++) {
			for(int j =  0; j<FMsketchMatrix.length/segmentLength; j++) {
				int k = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowid) ^ S[j] ^ SS[i]) % w + w) % w;
				for (int pp = 0; pp<segmentLength; pp++) {

					this.FMsketchMatrix[j*segmentLength+pp].and(fm[i].FMsketchMatrix[k*segmentLength+pp]);
				}
			}
		}
		return this;
