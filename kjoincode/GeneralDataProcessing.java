...
public class GeneralDataProcessing {
	public static String path = "D:\\GeneralFramework\\";		// absolute path for data
	...
	public static int numOfJointTimes = 4;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		}*/ //original process of data trace
		
		/*	for (int i = 0; i < 10; i++) {
				for (String flowLabel: flowLabels) {
					getPerSourceStatistics(Integer.toString(i)+"v", "", "", flowLabel);
				}
			}*/
	int[] j = {4,6,8};
for ( int i:j) {
	numOfJointTimes = i;
	System.out.println("src"+Integer.toString(i));
	getPerSourceStatistics(Integer.toString(i)+"v", "", "", "src");
	System.out.println("dst"+Integer.toString(i));

	getPerSourceStatistics(Integer.toString(i)+"v", "", "", "dst");
	...
	public static void getPerSourceStatistics(String date, String timeStamp, String routerName, String flowLabel) throws FileNotFoundException {
		...
		if (flowLabel.equals("src")) { 		//  per-source flow
			/** For flow size **/
			Scanner sc = new Scanner(new File("D:\\T"+Integer.toString(numOfJointTimes)+"\\srcSize.txt"));
			String outputSizeFileName = flowLabel + "Count" + ".txt";
			PrintWriter pw = new PrintWriter(new File("D:\\T"+Integer.toString(numOfJointTimes) + "\\" + outputSizeFileName));	
			...
			/** For flow spread **/
			sc = new Scanner(new File("D:\\T"+Integer.toString(numOfJointTimes)+"\\srcSpread.txt"));
			String outputSpreadFileName = flowLabel + "Card"+ ".txt";
			pw = new PrintWriter(new File("D:\\T"+Integer.toString(numOfJointTimes) + "\\" + outputSpreadFileName));	
			...
			/** For flow size **/
			Scanner sc = new Scanner(new File("D:\\T"+Integer.toString(numOfJointTimes)+"\\dstSize.txt"));
			String outputSizeFileName = flowLabel + "Count" + ".txt";
			PrintWriter pw = new PrintWriter(new File("D:\\T"+Integer.toString(numOfJointTimes) + "\\" + outputSizeFileName));	
			HashMap<String, Integer> mp = new HashMap<String, Integer>();
			while (sc.hasNext()) {
			...
			/** For flow spread **/
			sc = new Scanner(new File("D:\\T"+Integer.toString(numOfJointTimes)+"\\dstSpread.txt"));
			String outputSpreadFileName = flowLabel + "Card" + ".txt";
			pw = new PrintWriter(new File("D:\\T"+Integer.toString(numOfJointTimes) + "\\" + outputSpreadFileName));	
			HashMap<String, HashSet<String>> mp_spread = new HashMap<>();
			while (sc.hasNext()) {
			...
			/** For flow size **/
			Scanner sc = new Scanner(new File(path + "\\CAIDA" + routerName + "\\" + inputFileName));
			String outputSizeFileName = flowLabel + "Size" + date + timeStamp + ".txt";
			PrintWriter pw = new PrintWriter(new File(path + "\\CAIDA" + routerName + "\\" + outputSizeFileName));	
			HashMap<String, Integer> mp = new HashMap<String, Integer>();
			while (sc.hasNext()) {
				String entry = sc.nextLine();
				String[] strs = entry.split("\\s+");
	...
	public static void spatialJoinStatistics(String date, String timeStamp, Set<String> routerNames, String flowLabel) throws FileNotFoundException  {
		if (flowLabel.equals("src")) { 		//  per-source flow
			...
			for (int i=0;i<numOfJointTimes;i++) {
				System.out.println("src+size"+Integer.toString(i));
				String inputFileName = "output" + Integer.toString(i) + "v.txt";
				Scanner sc = new Scanner(new File(path + "\\CAIDA\\" + inputFileName));
				while (sc.hasNext()) {
					String entry = sc.nextLine();
					String[] strs = entry.split("\\s+");
					String srcIP = strs[0], dstIP = strs[1];
			...
			/** For flow spread **/
			for (int i=0;i<numOfJointTimes;i++) {
				System.out.println("src+spread"+Integer.toString(i));
			...
			for (int i=0;i<numOfJointTimes;i++) {
				System.out.println("dst+size"+Integer.toString(i));
			...
			/** For flow spread **/
			for (int i=0;i<numOfJointTimes;i++) {
				System.out.println("dst+spread"+Integer.toString(i));
